...
export function EditReleaseDialog({ release, onUpdate }: EditReleaseDialogProps) {
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  const [formData, setFormData] = useState({
    version: release.version,
    description: release.description,
    modelUpdate: release.modelUpdate,
    modelFileName: release.modelFileName || "",
    accuracy: release.accuracy.toString(),
    architecture: release.architecture,
    tp: release.tp.toString(),
    tn: release.tn.toString(),
    fp: release.fp.toString(),
    fn: release.fn.toString(),
    trainingTime: release.trainingTime.toString(),
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: `Successfully updated release ${formData.version}`,
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Release - {release.module}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-4">
            <Button type="submit" className="flex-1">Update Release</Button>
